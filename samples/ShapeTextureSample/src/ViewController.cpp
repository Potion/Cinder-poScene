...
namespace sample {
	...
	void ViewController::viewDidLoad()
	{
		...
		getView()->addSubview(mEllipseShape);
		...
		getView()->addSubview(mTriangleShape);
	}
	...
	void ViewController::createFitIndicators()
	{
		...
		getView()->addSubview(mFitIndicatorContainer);
	...
	void ViewController::createAlignmentIndicators()
	{
		...
		getView()->addSubview(mAlignIndicatorContainer);
		...
		for (int i = 0; i < mAlignIndicatorNames.size(); i++) {
			...
			IndicatorRef indicator = Indicator::create(indicatorText);
			mAlignIndicatorContainer->addSubview(indicator);
			if (i == 0) {
	...
	void ViewController::activateFitIndicator(int num)
	{
		for (int i = 0; i < mFitIndicatorNames.size(); i++) {
			if (i == num) {
	...
	void ViewController::activateAlignmentIndicator(int num)
	{
		for (int i = 0; i < mAlignIndicatorNames.size(); i++) {
			if (i == num) {
