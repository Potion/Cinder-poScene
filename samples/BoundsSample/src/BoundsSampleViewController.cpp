...
BoundsSampleViewControllerRef BoundsSampleViewController::create()
{
    BoundsSampleViewControllerRef ref(new BoundsSampleViewController());
    ref->setup();
    return ref;
}

void BoundsSampleViewController::setup()
{
    ...
    .getSignal(MouseEvent::Type::DOWN_INSIDE).connect(std::bind(&BoundsSampleViewController::nodeMouseOver, this, std::placeholders::_1));
    ...
    ci::app::getWindow()->getSignalKeyDown().connect(std::bind(&BoundsSampleViewController::keyPressed, this, std::placeholders::_1));
    
//    ShapeRef rect = Shape::createRect(400,400);
//    setMask(rect);
}

void BoundsSampleViewController::update()
{
    // Set the BGs
...
void BoundsSampleViewController::nodeMouseOver(po::scene::MouseEvent &event)
{
    if(mSelectedView)
    {
...
void BoundsSampleViewController::keyPressed(ci::app::KeyEvent &key)
{
    if(!mSelectedView)
        return;
...
std::string BoundsSampleViewController::getViewInfo(po::scene::ViewRef node)
{
    std::stringstream ss;
    ss << node->getName()
    << "\n---------------------------------"
